...
public class ExampleEntityEntity extends TameableEntity {
    public final AnimationState idleAnimationState = new AnimationState();
    public final AnimationState walkingAnimationState = new AnimationState();
    private int idleAnimationTimeout = 0;

    public ExampleEntityEntity(EntityType<? extends TameableEntity> entityType, World world) {
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        ExampleEntityEntity example_entityEntity = ModEntities.EXAMPLE_ENTITY.create(world, SpawnReason.BREEDING);
		if (this.isTamed()) {
			example_entityEntity.setOwner(this.getOwnerReference());
			example_entityEntity.setTamed(true, true);
		}
		return example_entityEntity;
    }
	...
	public boolean canBreedWith(AnimalEntity other) {
		if (other == this)
			...
			return false;
		else if (!(other instanceof ExampleEntityEntity example_entityEntity))
			return false;
		else if (!example_entityEntity.isTamed())
			return false;
		else
			return this.isInLove() && example_entityEntity.isInLove();
	}
}